...
<script type="text/javascript">
	function myCheck() {
		var using_amount = document.getElementsByName("using_amount");
		var amount = document.getElementsByName("amount");
		for (var i = 0; i < document.myform.elements.length - 1; i++) {
			if (using_amount[i].value > amount[i].value) {
				alert("可用数量小于输入数量");
				document.myform.elements[i].focus();
				return false;
			}
			if (using_amount[i].value == "") {
				alert("不可输入为空");
				document.myform.elements[i].focus();
				return false;
			}
...
<body bgcolor="#ebf3fb" oncontextmenu="event.returnValue=true"
	...
	<form name="myform" action="procedureAdd.do" method="post"
		onsubmit="return myCheck()">
		<table align=center class="TABLE_STYLE4">
			<tr height=20 class="TR_STYLE1">
				<td><a
					href="javascript:history.go(-1);opener.location.reload()">返回上一页</a></td>
				<td class="TD_STYLE3"><input name="productname" type="hidden"
					value="${dto.productName }">制造:${dto.productName }产品，供选择的物料清单如下：&nbsp;</td>
				<input name="productId" type="hidden" value="${dto.productId }">
				<input name="procedureId" type="hidden" value="${dto.procedureId }">
				<input name="detailsNumber" type="hidden"
					value="${dto.detailsNumber }">
				<input name="designId" type="hidden" value="${dto.designId }">
				<input name="procedureName" type="hidden"
					value="${dto.procedureName }">
		...
		<table bordercolor=#000000 bordercolorlight=#848284
			...
			<tr height=20 bgcolor=#D2E9FF class="TR_STYLE2">
				...
				<td bordercolor=#DEDBD6 align=left class="TD_STYLE2"><font
					class="resizeDivClass" onmousedown="MouseDownToResize(this);"
					onmousemove="MouseMoveToResize(this);"
					onmouseup="MouseUpToResize(this);"></font>可用数量</td>
				<td bordercolor=#DEDBD6 align=left class="TD_STYLE2"><font
					class="resizeDivClass" onmousedown="MouseDownToResize(this);"
					onmousemove="MouseMoveToResize(this);"
			...
			<tr height=20 class="TR_STYLE1">
				<c:forEach items="${procedure}" var="str" varStatus="rows">
					...
					<td bordercolor=#DEDBD6 align=left class="TD_STYLE2"><input
						name="amount" type="hidden" value="${str.amount }" id="amount">${str.amount }</td>
					<td bordercolor=#DEDBD6 align=left class="TD_STYLE2"><input
			...
			<td bordercolor=#DEDBD6 align=left class="TD_STYLE2"><input
				class="INPUT_STYLE5" type="submit"></td>
		</table>
		<table align=center class="TABLE_STYLE4">
			<tr height=20 class="TR_STYLE1">
